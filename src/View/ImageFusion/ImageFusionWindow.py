...
class UIImageFusionWindow(object):
    ...
    def on_loaded(self, results):
        """
                       ...
                       """
        try:
            print("TRACE: on_loaded called with results:", repr(results), "type:", type(results))
            if isinstance(results, tuple):
                print(
                    "TRACE: Tuple contents:",
                    [f"{repr(x)} (type: {str(type(x))})" for x in results],
                )
            """
                   Executes when the progress bar finishes loaded the selected files.
                   Emits a wrapper object that provides update_progress for compatibility with the main window.
            ...
            import traceback
            print("STACK TRACE: Exception in on_loaded:", e)
            print(traceback.format_exc())
            QMessageBox.critical(self.progress_window, "Fusion Error",
                                 f"Exception in on_loaded:\n{e}\n\n{traceback.format_exc()}")
